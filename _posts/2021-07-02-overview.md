...
### C++和C区别
- 语言设计理念
   C也可以通过一些技术模拟OOP，但这并不是语言本身的一部分。
- 应用
- 某些关键词的语义不同，如struct：
在C中，struct仅仅表示一堆数据的聚合，没有class-like features:没有方法，没有构造函数，没有基类。
C++继承了struct关键字，然后扩展了语义。

### C++里struct与class区别
成员的默认访问控制权限不同，在struct中是public，在class中是private.
何时用struct,何时用class：
...
## Linux进程间通信方式：PIPE,FIFO,消息队列，信号量，共享内存，socket
